import { Box, Button, Link, Image, Text, useColorModeValue, Flex } from '@chakra-ui/react';
...
function App() {
  const { data: count, isError, isLoading } = useContractRead({
  ...
  return (
    <>
      <Flex justify="space-between" align="center" p="1rem" boxShadow="md" bg="blue.500">
        <Text color="white" mr="1rem" fontSize="2xl" fontWeight="bold">Your dApp Name</Text>
        <ConnectButton />
      </Flex>
    <Box maxW="1280px" m="0 auto" p="2rem" textAlign="center">
      ...
      <Text fontSize="2xl">Vite + React + Bun</Text>
      <Box p="2em" borderRadius="md" boxShadow="md">

        <Button colorScheme="blue" onClick={() => increment ? increment() : null}>
          {isLoading ? (
            <Text>Loading...</Text>
          ) : isError ? (
            <Text>Error</Text>
